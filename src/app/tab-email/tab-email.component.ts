...
export class TabEmailComponent implements OnInit {
  faCheckCircle = faCheckCircle; // low
  faExclamationCircle = faExclamationCircle; // medium
  faTimesCircle = faTimesCircle; //high
  ...
  isLow(item: Email): boolean {
      return item.status ===  'low';
  }

  isMedium(item: Email): boolean {
    return item.status ===  'medium';
  }

  isHigh(item: Email): boolean {
